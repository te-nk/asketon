...
> _**Tip:** To bypass the root requirement, you can manually grant write permissions to the file once: `chmod 666 /etc/resolv.conf`. Use this only in isolated environments like iSH._

#  
:** Root privileges are required to modify `/etc/resolv.conf`.
* **Issue:** If running without sudo or root, the DNS auto-patcher will raise a `Permission Denied error`.
> _**Tip:** To bypass the root requirement, you can manually grant write permissions to the file once: `chmod 666 /etc/resolv.conf`. Use this only in isolated environments like iSH._
